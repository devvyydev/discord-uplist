  ...
  module.exports = async (client) => {
    ...
    passport.use(new Strategy({
      ...
      clientSecret: process.env.websitesecret,
    ...
    app.engine("uplist", ejs.renderFile);
    app.set("view engine", "uplist");
  ...
  app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://www.disc-uplist.xyz/user/${a.id}) just tried to log in to the site, but could not log on beacuse they are blocked or banned from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from uplist.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://www.disc-uplist.xyz/user/${a.id}) just logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    http.listen(80, () => { console.log("[uplist]: Website running on 80 port.")});
  ...
  const checkMaintence = async (req, res, next) => {
    ...
    sleep(1050)
    console.clear();
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: General routers loading...");
    sleep(500);
    ...
    /* Uptime System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Uptime system routers loading...");
    sleep(500);
    ...
    /* Profile System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Profile system routers loading...");
    sleep(500);
    ...
    /* Code Share System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Code Share system routers loading...");
    sleep(500);
    ...
    /* Botlist System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Botlist system routers loading...");
    sleep(500);
    ...
    /* Server List System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Serverlist system routers loading...");
    sleep(500);
    ...
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Admin Panel system routers loading...");
    sleep(500);
    ...
    /* Bot System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[uplist]: Bot system loading...");
