...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[uplist]: ${files.length} command loaded.`);
    files.forEach(f => {
...
/*=======================================================================================*/
const devvyy = require("./src/database/models/uptime.js")
    setInterval(() => {
        devvyy.find({}, function (err, docs) {
            ...
            docs.forEach(docs => {
                request(docs.link, async function(error, response, body) {
                  if(error) {
                    ...
                    await devvyy.findOneAndDelete({ code: docs.code })
                  }
                });
            })
        })
...
client.on('guildMemberRemove', async member => {
    if(member.guild.id !== config.serverID) return
        devvyy.find({ userID: member.id }, async function (err,docs) {
            await docs.forEach(async a => {
            await devvyy.findOneAndDelete({ userID: member.id, code: a.code, server: a.server, link: a.link })
            })
        })
    })
/*=======================================================================================*/
...
client.on('ready',async () => {
	client.channels.cache.get("902179722983788604").send(`current status!\n **online**\n visit our site at:\n https://www.disc-uplist.xyz/`);
    setInterval(async() => {
...
serverClient.login(process.env.tokenserver);
...
client.login(process.env.tokenbots);
client.on('ready',async () => {
    console.log("[uplist]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'uplist | '+bots.length+' bots' }, status: "online" });
});
/*=======================================================================================*/
